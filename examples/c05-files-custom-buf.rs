use std::{fs::File, io::{self, BufReader, Read}, path::Path, time::{Duration, Instant}};
...
fn hash_file(path: impl AsRef<Path>) -> Result<(u64, String, Duration)>{
    ...
    // Use custom buffer size to copy chunks directly into hasher
    const BUF_SIZE: usize = 8192;
    let mut buf = [0; BUF_SIZE];

    loop {
        let bytes_count = reader.read(&mut buf)?;
        if bytes_count == 0 {
            break; // EOF
        }

        // Update the hasher with the chunk read
        // Use slicing to only update with the actual bytes read
        hasher.update(&buf[..bytes_count]);
    }
